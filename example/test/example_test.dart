  ...
  group('example CLI', () {
    test('system version', () async {
      final result = await Process.run(
        ...
        ['run', 'example/example.dart', 'system', 'version'],
    ...
    test('user list', () async {
      final result = await Process.run(
        Platform.resolvedExecutable,
        ['run', 'example/example.dart', 'user', 'list'],
        workingDirectory: Directory.current.path,
      );
    ...
    test('user show 42', () async {
      final result = await Process.run(
        Platform.resolvedExecutable,
        ['run', 'example/example.dart', 'user', 'show', '42'],
        workingDirectory: Directory.current.path,
      );
    ...
    test('order process with flags', () async {
      final result = await Process.run(
        Platform.resolvedExecutable,
        ['run', 'example/example.dart', 'order', 'process', '900', '--dry-run', '--threads', '4'],
        workingDirectory: Directory.current.path,
      );

      expect(result.exitCode, equals(0));
      expect(result.stdout, contains('Orders.process(id=900, dryRun=true, threads=4)'));
    });
  });
}
